...
test_model=yumin_aug
tree_model=yumin_aug

#生成data的decode檔案
model_name=yumin_aug
dir=exp_learning/$model_name/chain
data=$(pwd)/data/yumin_aug/final
outside_dir=exp_learning/$model_name/chain_outside
#################################################################### for root

#################################################################### for test data
test_file=yumin_aug_outside
test_file_subdir=yumin_aug_outside
test_name=yumin_aug_outside
# test_name=MFCC${MFCC_dim}

test_data=$data_root/$test_file/$test_file_subdir/$test_name
...
##############################
steps/online/nnet3/decode.sh --acwt 1.0 --post-decode-acwt 10.0 --nj $nj --cmd run.pl $dir/graph $data/nopitch_inside ${outside_dir}_online/decode
##############################
echo -e "####PPG"
# simple PPG
